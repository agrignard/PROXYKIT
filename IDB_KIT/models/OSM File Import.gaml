...
experiment "Mirones" type: gui
{
	parameter "File:" var: osmfile <- file<geometry> (osm_file("../includes/mirones.osm"));//, filtering));
	output
	{
...
experiment "Redonda" type: gui
{
	parameter "File:" var: osmfile <- file<geometry> (osm_file("../includes/redonda.osm", filtering));
	output
	{
		display map type: opengl
		{
			...
			species road refresh: false;
			species node_agent refresh: false;
			species osm_agent refresh:false;
		}
